# 2.5.9 Topology Aware Routing — teaching transcript
...
**Prerequisites:** [2.5.5 EndpointSlices](../2.5.5-endpointslices/README.md); [2.5.1 Service](../2.5.1-service/README.md).
...
```bash
cd "$(git rev-parse --show-toplevel 2>/dev/null)/part-2-concepts/2.5-services-load-balancing-and-networking/2.5.9-topology-aware-routing" 2>/dev/null || cd .
```
...
- **Confusion with internal traffic policy** → see [2.5.12](../2.5.12-service-internal-traffic-policy/README.md) for **Local** semantics
