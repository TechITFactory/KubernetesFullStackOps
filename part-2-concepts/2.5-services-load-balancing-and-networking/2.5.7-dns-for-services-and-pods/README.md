# 2.5.7 DNS for Services and Pods — teaching transcript
...
```bash
cd "$(git rev-parse --show-toplevel 2>/dev/null)/part-2-concepts/2.5-services-load-balancing-and-networking/2.5.7-dns-for-services-and-pods" 2>/dev/null || cd .
```
...
- **NetworkPolicy blocks UDP/TCP 53** → allow egress to DNS Service ([2.5.6](../2.5.6-network-policies/README.md))
