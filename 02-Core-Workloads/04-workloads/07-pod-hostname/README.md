# Pod Hostname — teaching transcript
...
```bash
cd "$(git rev-parse --show-toplevel 2>/dev/null)/02-Core-Workloads/04-workloads/07-pod-hostname" 2>/dev/null || cd .
```
...
- **DNS does not resolve peer** → verify **headless Service** and **subdomain** alignment; see [2.4.3.3 StatefulSets](../../15-workload-management/18-statefulsets/README.md)
