...
Annotations are the other half of Kubernetes metadata — and they're commonly confused with labels.

The distinction is simple: **labels are for selecting, annotations are for describing**. A label is something you query on. An annotation is something you attach for tooling, documentation, or operational context. You can't use annotations in `kubectl get -l` selectors. You can't use them in Service selectors or Deployment ownership chains. But you can store anything in them — git commit SHAs, deploy timestamps, runbook URLs, Helm release metadata, Ingress controller directives.

Many Kubernetes features are controlled through annotations. The nginx Ingress controller reads `nginx.ingress.kubernetes.io/rewrite-target`. cert-manager reads `cert-manager.io/cluster-issuer`. Helm stores its release state in annotations. If you've ever wondered how those tools know what to do — annotations are how.
...
cd "$COURSE_DIR/C:/src/K8sOps/02-Core-Workloads/01-overview/07-annotations"
```
...
## Step 1 — Apply the annotated ConfigMap

**What happens when you run this:**
`kubectl apply -f yamls/annotations-demo.yaml` creates a ConfigMap with several annotations set — git SHA, deploy timestamp, runbook URL, and a tool-specific hint. Declarative; safe to re-run.
...
**What happens when you run this:**
`kubectl get cm annotations-demo -o jsonpath='{.metadata.annotations}'` prints the annotations map as raw JSON (may be long — `head -c 200` truncates). `kubectl describe cm annotations-demo` shows annotations in a more readable block format. Both are read-only.

**Say:**
Two ways to read annotations. `jsonpath` is precise — I can extract a single annotation value with `.metadata.annotations.<key>`. `describe` is readable — shows all annotations with their full keys in a block. I use `jsonpath` in scripts and `describe` when I'm debugging manually.
...
- **Annotation not affecting tool behavior** → check the exact key spelling; annotation keys are case-sensitive and domain-prefix must match exactly what the tool expects (e.g. `nginx.ingress.kubernetes.io/`, not `ingress.nginx.io/`)
- **`kubectl get -l annotation-key=value` returns nothing** → annotations cannot be used as label selectors; use labels for querying
- **`metadata.annotations` too large** → annotations over 256 KB total will be rejected; move large values to ConfigMaps
- **`kubectl apply` overwrites annotation added by a tool** → use `kubectl annotate` to add annotations that should persist outside your manifest, or add them to the manifest so they're tracked
...
**Say:**
The `describe` output shows annotations exactly as a human would read them during an incident. This is what I look at when I'm debugging "why is this Ingress not working" — I check the annotations first.
