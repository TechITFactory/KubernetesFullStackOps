...
```
  [ Step 1 ]                  [ Step 2 ]
  Apply notes      â†’          Inspect API versions
...
## Step 1 â€” Apply the storage version notes
...
**Say:**
I store these notes in `kube-system` so they're accessible to any engineer with cluster access. If your cluster's RBAC blocks writes to `kube-system`, apply to another namespace instead â€” the content is the same.
...
## Step 2 â€” Inspect API versions

**What happens when you run this:**
`kubectl api-versions` lists every API group and version the server currently serves â€” read-only. `kubectl get --raw /apis/apps/v1` fetches the raw API group definition JSON, showing what resources the `apps/v1` group exposes.

**Say:**
`kubectl api-versions` is how you confirm what your cluster actually serves right now. Before an upgrade, I run this, save the output, then compare after the upgrade. Any version in the pre-upgrade list that's gone post-upgrade means manifests using that version will break. The `--raw /apis/apps/v1` call shows the JSON underneath â€” it lists resources, verbs, and the storage version marker.
...
## API version deprecation timeline â€” what to watch for
...
- **`Forbidden` on `kube-system` write** â†’ change `namespace: kube-system` to `namespace: default` in the YAML, or apply with `-n default`; the content is unchanged
- **`api-versions` missing a version after upgrade** â†’ any manifest using that version will fail; update manifests to use the replacement version listed in K8s release notes
- **`kubectl convert` not available** â†’ it's a separate plugin since K8s 1.20; install with `kubectl krew install convert`; converts manifest files between API versions without hitting the cluster
- **Object stuck in storage with old version** â†’ the API server automatically serves the stored version converted to the requested version; if a migration is needed (rare), it happens on-read and re-write; check the K8s Storage Version Migration guide for the rare cases requiring explicit migration
...
**Say:**
`kubectl api-versions` is a quick cluster health diagnostic â€” I can see at a glance what API surface this cluster exposes. Then I clean up the ConfigMap. If you want to keep the notes, skip the delete.
