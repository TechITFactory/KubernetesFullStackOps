# 2.5.8 IPv4/IPv6 Dual-Stack â€” teaching transcript

## Intro

**Dual-stack** Kubernetes assigns both **IPv4** and **IPv6** addresses to **Pods** and **Services** when the control plane, **CNI**, and **node** networking are configured for it. On **Services**, **`ipFamilies`** and **`ipFamilyPolicy`** (`SingleStack`, `PreferDualStack`, `RequireDualStack`) control whether one or two **clusterIPs** are allocated; the **`clusterIPs`** field can hold multiple entries. **EndpointSlices** use **addressType** per sliceâ€”IPv4 and IPv6 backends may appear as **separate** slices. If the cluster is **IPv4-only**, manifests with **RequireDualStack** fail admission or Services stay single-stackâ€”always **`kubectl get svc kubernetes -o jsonpath='{.spec.clusterIPs}'`** on a real cluster before teaching guarantees.

**Prerequisites:** [2.5.7 DNS](../07-dns-for-services-and-pods/README.md); [2.5.5 EndpointSlices](../05-endpointslices/README.md) helpful.
...
```bash
cd "$(git rev-parse --show-toplevel 2>/dev/null)/part-2-concepts/2.5-services-load-balancing-and-networking/08-ipv4-ipv6-dual-stack" 2>/dev/null || cd .
```
...
## Step 2 â€” Inspect default Service cluster IPs

**What happens when you run this:**

**`kubernetes` default Service** shows how apiserver is exposed inside the clusterâ€”**one or two** IPs depending on configuration.
...
- **`RequireDualStack` rejected** â†’ cluster single-stack; change policy or enable dual-stack
- **Pods IPv6-only, app IPv4** â†’ app bind address or **happy eyeballs** issues
- **Slice addressType mismatch** â†’ CNI not programming both families on endpoints
- **DNS AAAA without route** â†’ clients try IPv6 firstâ€”validate end-to-end
- **kube-proxy mode differences** â†’ iptables vs IPVS vs eBPFâ€”vendor docs for dual-stack
