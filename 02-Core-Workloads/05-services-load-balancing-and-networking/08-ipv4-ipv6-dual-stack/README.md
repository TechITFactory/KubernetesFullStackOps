# IPv4/IPv6 Dual-Stack — teaching transcript
...
```bash
cd "$(git rev-parse --show-toplevel 2>/dev/null)/02-Core-Workloads/2.5-services-load-balancing-and-networking/08-ipv4-ipv6-dual-stack" 2>/dev/null || cd .
```
