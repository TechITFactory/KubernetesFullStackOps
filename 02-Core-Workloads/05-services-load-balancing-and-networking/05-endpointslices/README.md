# 2.5.5 EndpointSlices — teaching transcript
...
```bash
cd "$(git rev-parse --show-toplevel 2>/dev/null)/part-2-concepts/2.5-services-load-balancing-and-networking/2.5.5-endpointslices" 2>/dev/null || cd .
```
...
Shows slices for Services that have **ready** backends—**svc-demo** helps if [2.5.1](../2.5.1-service/README.md) lab ran.
...
- **Dual-stack skew** → **addressType** IPv4 vs IPv6 slices mismatch—see [2.5.8](../2.5.8-ipv4-ipv6-dual-stack/README.md)
