...
**Pick one runtime per node.** Lessons **01**, **02**, and **03** are **alternatives** — same teaching layout (**What happens → Say → Run → Expected**), different packages and sockets.
...
I stand here so links to **01**–**03** resolve the same way on camera.
...
- **Mixed instructions from multiple 01.x lessons** → pick one runtime; uninstall or reset the node before switching in a real cluster
...
| **03** | [Docker + cri-dockerd](03-docker-engine-via-cri-dockerd/README.md) | Legacy / migration only — not for greenfield. |
