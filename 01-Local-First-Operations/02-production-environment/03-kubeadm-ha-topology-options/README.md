...
Everything we have built so far has a single point of failure: the control-plane node. If that one server goes down — hardware failure, kernel panic, accidental shutdown — the entire cluster loses its brain.
...
### [0:45–2:00] What HA Actually Means for Kubernetes
...
The first three (API server, controller-manager, scheduler) are relatively easy to make highly available — they are stateless, so you can run multiple replicas. Only one controller-manager and one scheduler are active at a time (they use leader election), but multiple replicas are running and ready to take over.





**etcd is different.** etcd is a **distributed key-value store** that holds the entire cluster state — every pod, every service, every config, every secret. It is stateful, it uses a consensus algorithm (Raft) for consistency, and it requires a majority of members to be healthy to function.
...
### [2:00–4:30] Stacked etcd Topology
...
**Minimum: 3 control-plane nodes** — needed for etcd quorum (can tolerate 1 failure).
...
- Simpler infrastructure — fewer node types to manage.


- kubeadm handles everything — no separate etcd cluster to provision.


- Lower cost — fewer total servers.
...
- Scaling control-plane nodes means scaling etcd nodes simultaneously — you cannot scale them independently.
...
### [4:30–6:00] External etcd Topology
...
│  (API server, ctrl-mgr, scheduler — no etcd)           │
...
- Separate failure domains — a control-plane problem cannot take down etcd and vice versa.


- Independent scaling — you can add control-plane nodes without touching etcd, and vice versa.
...
- More infrastructure — six or more nodes vs three.


- More operational complexity — separate certificate management, separate backup procedures, separate monitoring.


- kubeadm does not manage the etcd cluster — you provision and maintain it separately.
...
### [6:00–7:15] The Load Balancer Requirement
...
The load balancer provides a stable endpoint (`CONTROL_PLANE_ENDPOINT`) that does not change when individual control-plane nodes are added, removed, or replaced. Worker nodes and kubectl both talk to this endpoint — they do not talk directly to individual control-plane nodes.
...
### [7:15–8:15] Quorum — Why Odd Numbers
...
Notice: 4 members tolerate 1 failure — same as 3. 6 members tolerate 2 failures — same as 5. **Even numbers buy you nothing.** Always use odd numbers: 3, 5, or 7.
...
### [8:15–9:30] Choosing Your Topology
...
├── No  → Stacked etcd (3 control-plane nodes)


└── Yes → Do you have strict separation-of-concerns requirements?


          ├── No  → Stacked etcd (simpler, cheaper)


          └── Yes → External etcd (3 control-plane + 3 etcd nodes)
...
Next: 02.1.6 — Creating Highly Available Clusters with kubeadm, where we run the stacked HA init and join additional control-plane nodes.
