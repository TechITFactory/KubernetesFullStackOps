...
You will **either** use **Minikube (01.1)** **or** **Kind (01.2)** — pick one and stay on it. Then **03** adds a governed workspace namespace (`dev-local`) on top of whichever cluster you chose.
...
I stand in the module folder so relative links to **01.1**–**03** match what you see on screen.
...
- **Started both Minikube and Kind “just to compare”** → pick one context with `kubectl config use-context` before **03** or tear down the extra cluster to avoid accidental applies

- **`kubectl: command not found` before 01** → complete the installer steps inside **01.1** or **01.2** first

- **`dev-local` missing after 03** → re-run bootstrap from **03** on the correct context (`minikube` vs `kind-kfsops-kind`)
...
| **01.1** | [Minikube setup](01.1-minikube-setup-and-configuration/README.md) | You want one node, addons (e.g. Ingress), familiar “minikube service” flow. |

| **01.2** | [Kind](01.2-kind-kubernetes-in-docker/README.md) | You want multi-node-in-Docker, fast spin-up, CI-like workflows. |

| **03** | [Local dev clusters / dev-local](03-local-development-clusters/README.md) | **After** 01.1 or 01.2 — quotas, limit ranges, demo app in `dev-local`. |
